...
## GDD section 5's five survival readings, for UI design document section 5.2's
## notes and section 6.1's ring.
##
## Run: godot --headless --path <project> --script res://tools/generate_vital_layout.gd
##
## ---------------------------------------------------------------------------
## SIX STAT FILES, FIVE ROWS, AND WHY THAT IS NOT A MISTAKE
## ---------------------------------------------------------------------------
...
##
## So frostbite is ONE row carrying a `second_stat`, and VitalReadings folds the
## pair by showing the WORSE -- a player acts on whichever limb is further gone.
##
## ---------------------------------------------------------------------------
## THERE IS NO DAY DIAL IN THIS TABLE ANY MORE
## ---------------------------------------------------------------------------
## A sixth row used to sit at its head: `daylight`, sourced from WorldClock
## rather than from the survival model, drawn largest and the only warm one. It
## belonged to the permanent corner cluster, which the owner deleted -- and with
## it went `source`, `glyph_night`, the DIAL anchor and `warm_source`, every one
## of which existed for that row alone.
##
## The clock did not go with it. It is section 5.10's periodic time prompt now --
## bottom centre, every four hours, four seconds, gone -- which is the same
## information under rule 4 instead of against it.
...
## [stat, second_stat, label, limb, order, track_weight, ring_degrees,
##  ring_anchor, glyph].
##
## `track_weight` is the reading hierarchy: section 6.1's 110-against-58 as a
## ratio. 体温 是主时钟，是唯一致死的一条，层级必须在视觉上说出来 -- and a reading
## that is merely first in a list does not say it.
const TABLE := [
	[&"core_temperature", &"", "体温", "", 0, 1.9, 110.0, RING, &"core_temperature"],
	[&"hunger", &"", "饥饿", "", 1, 1.0, 58.0, RING, &"hunger"],
	[&"thirst", &"", "口渴", "", 2, 1.0, 58.0, RING, &"thirst"],
	[&"fatigue", &"", "疲劳", "", 3, 1.0, 58.0, RING, &"fatigue"],
	[&"frostbite_hands", &"frostbite_feet", "冻伤", "", 4, 1.0, 34.0, HANDS,
		&"frostbite_hands"],
]

## Section 6.1's Tab ring: 段间空隙 14°, 半径 = 1.6 × 角色屏幕高度.
...
func _initialize() -> void:
	...
	for entry in TABLE:
		...
		row.stat = entry[0]
		row.second_stat = entry[1]
		row.label = entry[2]
		row.limb = entry[3]
		row.order = int(entry[4])
		row.track_weight = float(entry[5])
		row.ring_degrees = float(entry[6])
		row.ring_anchor = int(entry[7])
		row.glyph = entry[8]
		rows.append(row)
	...
	for row in written.ordered():
		print("  %d  %-18s %-6s track ×%.2f  glyph %s%s" % [
