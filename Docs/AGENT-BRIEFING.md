...
### 重定向：每一个关节角度都对到零点一度，姿势仍然是错的

把一个 take 从别的骨骼重定向过来时，**第一次烘焙交付了 10.5°，而设计要的是 26.8°——同时每一个关节角度读出来都正确到零点一度。**

原因：**两套骨骼把脊柱骨放在身体的不同高度上。**同样的角度施加在不同的力臂上，末端位移就不一样。角度是对的，姿势不是。

**为什么它会骗过人**：验收的直觉是去查角度，而角度是这条链上唯一一个**两边都能读、且读出来会相等**的量。你查到它相等，就会认为重定向成功了。

**怎么办：验末端，不验角度。**量那个你真正要的可观察结果——身体前倾了多少厘米、头低了多少、在游戏机位下轮廓变了多少像素——然后拿它去对设计值。这条和「设进去了 ≠ 起作用了」是同一条规律的两次发作：**断言那个下游的量。**

顺带一条同源的：这次的 26.7° 和方案里的 34.9° 不矛盾。**34.9° 是绝对倾角，其中 8.1° 是那具供体骨骼自己的中立含胸**；真正被创作出来的差值是 26.8°，而它到达时误差 0.1°。**报一个姿势的时候要说清它是绝对量还是差值量**，否则下一个人会拿它去追一个不存在的缺口。

### Trap 19 — 每一个"多远"都是从**监听器**量的，而监听器在九十米以外

所有者两次问「为什么现在还听不到乌鸦的叫声呢」。三个 `.wav` 在盘上、导入过、事件发出来了、节奏排出来了、发声体建出来了、`playing` 报 `true`、位置每帧跟着鸟走，**1930 个测试全绿**。

主总线峰值：**−200.00 dB。** 数字意义上的绝对静音，一个采样都没有。

**成因。** `AudioStreamPlayer3D.max_distance` 是**硬截断**，而它量的是**到监听器**的距离。监听器是当前的 `Camera3D`。这个项目的相机骑在 90 米的摇臂上——而在正交投影下**摇臂长度对画面完全没有影响**（`camera_rig.gd:33` 自己写着，它存在只是为了把世界推到近裁剪面前面）。于是一个纯粹为了裁剪面选的数字，悄悄变成了整个游戏被聆听的距离。

实测（真实场景，同一声音，只改一个属性）：

| 摆在哪 | 到监听器 | 主总线峰值 |
|---|---|---|
| 玩家身上，`max_distance = 60`（出厂设置） | 90.71 m | **−200.00 dB** |
| 玩家身上，`max_distance = 0`（取消截断） | 90.71 m | −36.12 dB |
| **监听器身上**，`max_distance = 60` | 0.00 m | **−8.60 dB** |
| 把监听器搬到玩家身上，其余不动 | — | **−8.74 dB** |

**听觉是反的。**同一声音摆在相机上响亮，摆在玩家脚边绝对静音——今天这套几何下能被听见的，是**身后三十到九十米、正好朝相机方向**的东西。

**波及面：凡是声明了自己能传多远的定位音效，全部静音。** 乌鸦 60 m、狗吠 60 m、呜咽 45 m、哀鸣 35 m、低吼 14 m、炉火 24 m。**唯一活下来的是环境风声床——因为它是唯一一个没有声明 `max_distance` 的。说得越具体，死得越彻底。**

**为什么它骗过了所有人。** 这个项目能拿出的最强乐器是 `is_playing()`，而它证明的是**音频服务器接受了这次播放**，不是**有采样到了输出**。查到这条为止，`get_bus_peak_volume` 和 `AudioListener3D` 这两个词在整个仓库里各出现 **0 次**——**没有人问过耳朵在哪。**

这是「设进去了 ≠ 起作用了」的**下一格**：

```
第一格   play() 返回了 true          （44 次假通过那回）
第二格   服务器接受了、is_playing()   （今天的套件停在这里）
第三格   有声音响了                   （从来没有人测过）
```

**通则，比音频宽得多：凡是衰减、LOD、剔除、可见性、细节层级、音量、雾、遮挡——都是相对于一个「观察者」算的，而那个观察者的位置常常是别的系统为了别的理由定的。**先问观察者在哪，再去信任任何一个"多远"。

并且这同时是「**一个必须与别的值相关的常量，要对着那个值测**」的又一次发作：`boom_length` 和那六个传播距离是**一组关系**，却被写成了六个互不相干的常量，而这个关系只存在于没有人的脑子里——所以摇臂可以被随便改，六个声音会一个一个悄悄消失，没有一条测试会红。

**乐器（这个项目第一次有）：**

```gdscript
AudioServer.get_bus_peak_volume_left_db(0, 0)   # 总线上真实的电平
```

无头模式下音频驱动是 `Dummy`，**但混音器照常跑，峰值表读数有效**——上表 −8.60 dB 的对照组就是它有效的证据。所以"有没有声音"从今天起是可以在套件里断言的，代价是一个真实场景和几秒钟。**断言这个，不要断言 `play()` 的返回值。**

全文与给所有者的解释在 `.superpowers/sdd/wave3/task-w3-bird-system-explained.md`。**根因未修**：改法是一个 `AudioListener3D`，但它会一次性重新调整全游戏的定位音效电平，属于所有者拍板的范围。

### 不对称只有在机位本身对称时才是绝对读数

Director 曾主张：**跛行是一种不对称，而不对称是绝对的**——一帧、画面里只有一个人就能看出来，正如佝偻之于饥饿。

**实测推翻。**这个游戏的机位是**四分之三后方**，而那个视角本身就不对称：**一个左右完全对称的走路，在这台相机下已经量出 4.38 px 的「不对称」。**而真正跛的那个 take（`Injured_Walk`，抬脚比 0.238）在同一台相机下的全部绝对不对称，是 **2.25 px 的头部起伏**——**比机位自己制造的假不对称还小。**

**所以：一个属性是不是「绝对读数」，取决于机位，不取决于那个属性。**在斜后方看，左右差被投影压掉；在正侧方看，它是最强的信号。**判断可读性之前，先问这台相机保留了哪个轴。**

同一次测量还推翻了当初的另一条论据。选择双侧步态时用的是**生理学**（冻伤伤及双脚，所以单侧跛是错的），而两条读数在这里被认为互斥。**实测两条都不成立**：不对称的那个读不出来（0.83），而两个**对称**的 take 读得出来（`crouch_creep` 1.50、`crouch_walk_forward` 1.43）。

> **这里的可读性来自「被端着的体态」，不来自不对称。**不必在一只脚和两只脚之间选。

**还有一条方法论，来自做这次测量的 agent 自己**：它的对照实验显示**跑 vs 走只得 0.23**，而那是每个玩家都看得见的差别。**所以这把尺子量的是「单帧、无参照的可读性」——对姿态是对的，对步频是错的。**它据此把结论从「看不见」收窄成「相对」，并主动报了出来。**一把尺子的适用范围，应该由造尺子的人说出来。**
