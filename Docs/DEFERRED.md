...
| ~~W3-8~~ | **PARTLY CLOSED.** The ambience takes have been cut into seamless loops in `assets/audio/ambience/` — `wind_low` 60.0 s, `wind_mid` 60.0 s, `wind_high` 4.6 s, `snow_fall` 5.4 s, `fire` 12.0 s, all 16-bit mono PCM with `edit/loop_mode` set to Forward and the join verified against each file's own adjacent-sample distribution. **Still owed: the six weather one-shots** in `assets/audio/weather/`, named `weather_tell_<event id>` for the six events in `data/weather/`. Also still owed and unchanged: the caws in `assets/audio/wildlife/crow/`. |
| ~~W3-10~~ | **CLOSED.** The nasal 3.6 s `wind_mid` and the breathing low bed were replaced by separate, non-overlapping 60 s passages from lwdickens' CC0 “winter wind in trees” (Freesound 261226). Both retain the existing `-20 dBFS` RMS target and use verified 10 s seamless crossfades; narrow fixed tones are removed without flattening the broadband air. |
...
| # | Finding |
|---|---|
| ~~W4-1~~ | **Closed — the “no middle” premise was a measurement error (2026-08-13).** The old report preserved an 80 m / 11,449-point (0.75 m) point sample but not its 12 m-window method. Reconstructing that lattice with `tools/snow_depth_scan.gd --span 80 --sample 0.75` reproduces the useful part of its observation — **40.55%** is at or past the 0.42 m wading gate — but not the claimed **35% bare** share (zero-depth is 24.64%) and it finds **3,697/8,464** sliding 12 m windows with means in 0.02–0.32 m. The standard 0.5 m scan finds **2,082/4,761** such windows in the same 80 m area and **5,468/11,881** across the 120 m field. `test_12m_windows_include_an_intermediate_mean_depth` now guards the factual absence claim without inventing an unapproved balance target. Point depths remain uneven, so Wave 4's run seeds, dynamic layers and later consumer tuning remain planned work; they must use owner-approved distribution and Day-1-route thresholds rather than this false binary diagnosis. |
...
## 更正 — 第七类假通过的机制记错了，修法随之作废（`1d0e156` 实测推翻）

上面那条「框架 — 第七类假通过」写着**着色器在更晚的一帧编译，而整个套件活在第一帧之内**，并据此开出「只需一行 `RenderingServer.force_draw()`」的修法。

**机制是错的，因此那一行什么都关不上。**着色器在**资源加载时同步编译**（`shader_set_code`，dummy 驱动同样如此），跟帧没有关系。验证：把当初那个 `return`-in-`sky()` 放回 `aurora_sky.gdshader`，**在 HEAD 上、不改任何框架代码，套件直接变红**。`RenderingServer.force_draw()` 在无头模式下静默运行，在一个已经坏掉的着色器旁边一个字都不多打，**按这份派发自己的标准（门禁必须被证明会响），它不该被加上**——所以没加。

**当初那个观测本身是真的**，一个死掉的着色器确实全绿通过了。**真因更窄也更糟：没有任何测试加载的着色器，根本不会被解析。**九个全部弄坏，七个报错、两个一声不吭（`chimney_smoke.gdshader` 与 `montage_film.gdshader`）——另外七个是被那些构建其宿主系统的测试顺带编译的。

同一次测出的另一件事同样值得留着：**九个全死的时候，运行器仍然读作 `1842 passed, 0 failed`。**每一条错误都是**控制台门禁**抓到的，**没有任何一个断言抓到它**。这道门禁的价值比断言大，别为了让运行变绿去拓宽它的白名单。

**已落地的修法**：`tests/art/test_shader_compiles.gd`，逐个以 `CACHE_MODE_IGNORE` 加载全部 `.gdshader`，并**从资源本身读回失败**——编译失败的着色器报告 0 个 uniform，正常的报告 38——因为引擎的报错**只给行号不给文件名**（`at: (null) (:353)`）。四次运行全部记录在案：坏着色器在门禁之前 `1842 passed / 0 failed / 退出 0 / 控制台干净`（就是那个洞），在门禁之后 `1843 passed / 2 failed / 退出 1` 并指名文件。

**当前产出**：门禁上线后**一个都没抓到**，九个着色器全部通过，两个 addon 着色器另行验证也通过。产量很薄，因为暴露面本来就只有那两个没被任何测试加载的。

**同轮附带**：词汇规则现在也扫 `res://src` 下每个 `.gd` 的**源码**，而不只是七个系统组成的图——**结果是 11 个脚本声明了风的词汇，而图里只登记了 3 个**。`WindSystem` 按方法名抢占的那个缺陷，规模是已知的 3.7 倍。`_step()` 与真实 tick 的漂移未取，理由是去掉这层模拟需要真实的帧边界，而套件按设计活在一个 `_process()` 里。

**这条更正本身的教训**已写进简报：一条记录里「测到了什么」和「为什么会这样」的可靠度不一样，而修法继承的是后者。

---

## 更正 — `shadow_normal_bias = 2.0` 不是病因，它是解药（`21d9753` 实测反转）

这一条一直登记在「等所有者裁决」里，措辞是：**`shadow_normal_bias = 2.0` 是白面上阴影接缝的根因，改它影响全场光照。**它等了很久，等的是证据。

**证据来了，方向是反的。**

把它调到 **0.1** 之后实测：一块**没有任何投射物**的裸雪，细结构能量是 2.0 时的 **29 倍**；整帧有 **4.1%** 的光被条纹吃掉；黎明时 `p90−p10` 上升 **40%**。

也就是说，**2.0 正是在压住那些接缝，而不是造成它们。**

同一次测量还澄清了一件被误认的事：两个取值下**接触距离没有变化**，所以**靴子附近那圈亮边是足迹的边缘，不是阴影脱离（peter-panning）。**

**这条裁决按现有证据可以关闭：保持 2.0，不要动它。**

**唯一的保留**：证据是**两个点，不是一次扫描**。如果将来有人要重开这件事，先做一次 0.1 → 2.0 的连续扫描，而不是再拿两个端点争论。做这次测量的 agent 给测量工具加了一个 `--bias` 旋钮，只在工具里，不写回项目。

**顺带记下这条为什么值得留着**：这条登记项存在期间，没有人怀疑过它的方向。它被写下来时是一个合理的推断——白面上有接缝，而 bias 是最相关的那个参数——**而推断和测量的可靠度不一样**，这正是简报里那条方法论的又一次发作。

---

## 所有者裁决 — 步态这个频道归疲劳（`27c00a7` 的测量引出）

**为什么只能给一条**：实测走路这个动作本身每帧搅动 **3895 px（占身形 44.9%）**，峰值 4863；腰以上 882 / 1186。**站姿频道的噪声底只有 640 / 868——走路频道是它的六倍。**一个频道的噪声这么大，塞两条读数的结果是两条都读不出来。

**为什么是疲劳**：五条状态里，**疲劳是唯一一个两条支路都看不见的**——站姿距离健康人 **0 px**（实测，像素级相同），步态 **0.23**。它是这具身体上最弱的一条。而所有者要导出的【老人走路】恰好是**"被端着的体态"**，那正是这次测量证明有效的那种可读性（不对称无效：0.83；对称的端着体态有效：`crouch_creep` 1.50、`crouch_walk_forward` 1.43）。

**替补必须过的线，已经是一个数**：腰以上最易混帧 **> 1186 px**（比值 ≥ 1.0），且作者速度 **≥ 0.90 m/s** 以留在 `anim_max_pace = 1.5` 之内。现有的 `walk_guarded` 是 565 px / 0.924 m/s。用 `--mode ceiling` 把候选放进 `CEILING` 跑一遍即可，一次出结果。

### 遗留：足部冻伤因此没有可读的身体读数

这条不是被解决了，是**被明确地留空了**，记在这里以免以后被误以为已完成。

现状：`walk_guarded` 是**相对读数**——单帧无参照读不出来（0.48），看久了未必。它没有被换掉，理由是实测能读的两个 take 说的是**「在潜行」**而不是「冻伤了」，**用一个看得见的错读换掉一个看不见的对读是亏的。**

三条已被排除的路，不要重走：**跛行不行**（`Injured_Walk` 需 1.53–2.58 倍速，且就算正常播也只得 0.83）；**不对称本身不行**（这台四分之三后方的相机，对称走路已自带 4.38 px 假不对称，而真跛只有 2.25 px）；**生理学的双侧论证是伪矛盾**（对称的端着体态才是有效的那一种）。

**没走过的路**：换机位不在讨论范围，但那条测量指出「绝对性取决于机位保留了哪个轴」——如果将来有某个镜头是正侧方的，左右差在那里是最强信号。以及冻伤本来就有火边治疗这个动作，**读数不一定非要长在步态上。**

---

## 暂停菜单打磨收尾（2026-08-13，排版层级刷新那轮登记）
