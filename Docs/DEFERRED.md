...
| W3-8 | **The ambience ships silent, and the shortfall is eleven files.** `AmbienceDirector` decides a whole bed every frame and plays nothing, because none of the audio exists. Owed, by folder and role: `assets/audio/ambience/` — `wind_low`, `wind_mid`, `wind_high`, `snow_fall`, `fire` (five seamless loops); `assets/audio/weather/` — six one-shots named `weather_tell_<event id>` for the six events already in `data/weather/`. **No `.gd` or `.tres` change is needed when they land** — both folders are resolved by name (`AmbienceMap.find()`), the same shape `CrowCalls` uses. Also still owed and unchanged: the caws in `assets/audio/wildlife/crow/`, which `CrowCalls` has been waiting on since Wave 2. |
