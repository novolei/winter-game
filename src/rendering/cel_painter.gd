...
## cross-section to show. The run opens at cover 0.62, but that must look like a
## young layer rather than a roof that is already finished: the modelled lip is
## only about one sixth grown there, reaches about seven tenths at cover 0.80,
## and does not finish until the late-winter ceiling. The ordinary shader snow
## on props keeps reading the unmodified cover; this later curve belongs only to
## the roof mass.
##
## `smoothstep` keeps the growth continuous at both ends, so neither the onset
## nor the final settling can produce one frame of popping.
const SNOW_MASS_ONSET := 0.50
const SNOW_MASS_FULL := 0.98
...
static func snow_mass(cover: float) -> float:
	return smoothstep(SNOW_MASS_ONSET, SNOW_MASS_FULL, clampf(cover, 0.0, 1.0))
...
func material_for(
	...
	snow_receptivity_scale := 1.0,
	snow_threshold_bias := 0.0,
	source_albedo_tint := Color.WHITE,
	source_albedo_tint_strength := 0.0
) -> ShaderMaterial:
	var source_key := ""
	...
	var key := "%s|%s|%s|%.3f|%.3f|%s|%.3f" % [
		lit.to_html(false), bare, source_key, snow_receptivity_scale, snow_threshold_bias,
		source_albedo_tint.to_html(false), source_albedo_tint_strength
	]
	...
	material.set_shader_parameter("source_albedo_tint", source_albedo_tint)
	material.set_shader_parameter("source_albedo_tint_strength", clampf(source_albedo_tint_strength, 0.0, 1.0))
