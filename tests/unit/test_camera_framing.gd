...
func _live_rig(with_camera := false) -> CameraRig:
	var rig := _rig()
	if with_camera:
...
## The tight stop must stay exactly what shipped, or every number tuned against
## it -- the terrain wavelength, the sun elevation, the print scale -- is tuned
## against something that no longer exists.
func test_the_frame_still_opens_where_it_always_did() -> void:
	var rig := _rig()
	assert_almost_eq(rig.orthographic_size, TIGHT, 0.0001, "the authored frame")
	assert_eq(rig.framing_index(), 0, "and it is the first stop, not a value between stops")
...
func test_the_camera_is_given_the_frame_the_rig_resolved() -> void:
	...
	assert_almost_eq(camera.size, TIGHT, 0.0001, "the camera opens at the authored stop")
	rig.cycle_framing(1)
	var tween := rig.framing_tween()
	if tween != null:
